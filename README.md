...
Triggers are special objects within ArmA, they get **triggered** and they **fire**. But they get triggered by what? And fire what precisely?

Perhaps we want code that should not be running as the mission starts, but rather we want this code to run **when a given condition is encountered** and perhaps we don't want it just to happen, but we want that condition to be met **only within a specific area**.

This is done by using triggers, they are **active listeners**: they can be positioned, they have a scalable volume and take a condition which is continuously evaluated to see if it is either **true** or **false**:
...
You can find an extensive explanation of what we are talking about really is [HERE](https://community.bistudio.com/wiki/Description.ext), I won't judge you if you don't want to read it. It is enough to know that within here, all the mission parameters are contained, such as:
...
The **description.ext** file allows us to modify some Configs, you don't know what configs are yet? Don't worry, it's not a big deal. Just know that they are lists of contents, which can be weapons (*cfgWeapons*), vehicles (*cfgVehicles*) and so on.

This has major implications: it means that we can add custom music by placing it in the mission folder and defining it in the **description.ext** as *cfgMusic*! Or even better, we can define some of our code as functions with *cfgFunctions*, which means that we can call our code whenever we want during the game, without having to write it all out or linking it from a file.

It would be very bad if we had a huge mission with tons of scripts all being linked directly in this file, that is why we use **#include**, which can link external config files (within our mission folder) to our existing description.ext, without encumbering it.
...
### Back to the mission files

Let's clear up what these five different files do:
* **init.sqf**: it does the same thing as a normal init field, but it is not bound to an object. It is run once at the very start of the mission.
* **initPlayerLocal.sqf**: if you got the concept of locality right, you might understand that if you want to run code that is not supposed to run on the server machine, then you should put it in here;
* **initServer.sqf**: again, this follows the locality principles;
* **onPlayerKilled.sqf**: is code that runs whenever the player dies;
